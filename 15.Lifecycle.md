...
## 7.5. Asignar el secret al Pod webapp
...
## 8.1. Ejecutar el ***liveness probe***
...
livenessProbe:
	...
	periodSeconds: 5
```
## 8.2. Ejecutar: ambos Pods están programados para levantar satisfactoriamente y después de unos segundos enviar una señal fallida para validar su comportamiento.
```vim
kubectl apply -f kubelabs-files-demo/pod-liveness.yml
```
## 8.3. Validar resultados:
```vim
...
## 8.5. Ejecutar el ***readiness probe***
...
readinessProbe:
  ...
  periodSeconds: 5
```
## 8.6. Ejecutar: El contenedor está programado con un delay de 60 segundos de arranque.
```vim
kubectl apply -f kubelabs-files-demo/pod-readiness.yml
```
## 8.7. Validar resultados:
...
## 10.3. En otra terminal, generar tráfico al Pod configurado
```vim
...
## 10.4. Detener el pod de carga y limpiar
