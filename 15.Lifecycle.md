...
### 9.5.1. Crear el archivo: secret-demo-webapp.yaml
```yaml
apiVersion: v1
kind: Pod
metadata:
...
```vim
kubectl apply -f assets/15/secret-demo-webapp.yaml
```
...
```vim
kubectl apply -f assets/pod-liveness.yaml
```
...
```vim
kubectl apply -f assets/pod-readiness.yaml
```
...
kubectl delete all --all
```
## 13.1. TIP: usar explain para revisar la configuración
```vim
