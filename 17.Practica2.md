...
## 3.2. Se debe acceder por el dominio USERLAB.kubelabs.dev (online ó .dev))

## 3.3. Es una página web reactiva que para este caso, debe ser accedida por HTTP (puerto 80) y su contenedor escucha por el puerto 8080.

## 3.4. Necesita una ruta apuntando a su root (/)

## 3.5. Para efectos de alta disponibilidad se requieren 2 replicas
## 3.6. Dirección de la imagen:
> cachac/kubelabs_webapp:1.1.7

## 3.7. La página requiere montar un archivo de configuración con las siguientes características:
### 3.7.1. Ubicación del archivo en la máquina virtual Bastion (máquina de trabajo):
> /home/kube/kubelabs/config.js

### 3.7.2. Editar el archivo config.js agregando el subdominio de cada estudiante
...
## 4.2. Este servicio se accede por la ruta websocket.USERLAB.kubelabs.com
...
## 5.2. Este servicio se accede por la ruta api.USERLAB.kubelabs.com
...
## 6.2. Este servicio privado que no se publica por Internet. Se accede a él únicamente a través del public api, esta conexión se debe configurar en el punto 6.
...
# 7. Establecer comunicación entre API público y privado
